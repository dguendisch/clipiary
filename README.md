...
```sh
./scripts/package_release.sh 0.3.2
```

That command writes:

- `dist/Clipiary-0.3.2.zip`
- `dist/Clipiary-0.3.2.sha256`
- `dist/clipiary.rb`

### CI release flow

The workflow in `.github/workflows/release.yml` is tag-driven and release-only. Pushing a tag such as `v0.3.2` from a commit on `main` will:
